...
pid_t ZygoteHostImpl::LaunchZygote(
  ...
  if (use_flatpak_sandbox_) {
    pid = sandbox::FlatpakSandbox::GetInstance()->LaunchProcess(
        *cmd_line, options);
  ...
  if (use_namespace_sandbox_ || use_suid_sandbox_ || use_flatpak_sandbox_) {
    // The namespace, SUID, and Flaptpak sandboxes will execute the zygote
    // in a new PID namespace, and the main zygote process will then fork
    // from  there. Watch now our elaborate dance to find and validate the
    // zygote's PID.
