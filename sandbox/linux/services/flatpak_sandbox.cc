...
FlatpakSandbox::SandboxLevel FlatpakSandbox::GetSandboxLevel() {
  ...
  if (sandbox_level_ == SandboxLevel::kFlatpak
      && base::CommandLine::ForCurrentProcess()->HasSwitch(kDisableFullFlatpakSandbox)) {
    sandbox_level_ = SandboxLevel::kRestricted;
  }
#endif
...
bool FlatpakSandbox::SendSignal(base::ProcessId external_pid, int signal,
...
void FlatpakSandbox::InitializeBusThread() {
  ...
  CHECK(properties.version.value() >= 4)
    << "Your Flatpak version is too old, please update it";
  CHECK(properties.supports.value() &
        PortalProperties::kFlatpakPortal_ExposePids)
      << "Your Flatpak installation is setuid, which is not supported";
...
void FlatpakSandbox::OnSpawnStartedSignal(dbus::Signal* signal) {
  ...
  auto it = running_processes_.find(external_pid);
  if (it == running_processes_.end()) {
    LOG(ERROR) << "Process " << external_pid << " is already dead or not tracked";
    return;
  }
...
void FlatpakSandbox::LaunchProcessOnBusThread(
    base::ProcessId* out_external_pid, base::WaitableEvent* event,
    const base::CommandLine& cmdline, const base::LaunchOptions& launch_options,
    bool allow_x11) {
  dbus::ObjectProxy* object_proxy = GetPortalObjectProxy();
